...
func _ready() -> void:
    panel.visible = not auto_hide_when_inactive
    label.text = ""
...
func start(dialogue_id: String) -> void:
    lines = DIALOGUES.get(dialogue_id, []) as Array[String]
    if lines.is_empty():
        push_warning("DialogueManager: unknown or empty dialogue_id: %s" % dialogue_id)
        stop()
        return
